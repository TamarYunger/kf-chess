...
full startup + call-flow traces for **all four entry points**
...
text-mode startup + command dispatch; `server/ws_server.py`'s own
startup, from `if __name__` through `serve_forever`'s listen+tick loop —
the server process itself, not a client), plus every file's responsibility
