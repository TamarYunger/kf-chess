...
## 7. קפיצה עם גובה אמיתי

לפני זה, קפיצה לא זזה אנכית בכלל — רק אנימציית הנדנוד ניגנה במקום. נוספה
`jump_height_offset` — קשת סינוס (0 בהמראה, שיא באמצע, 0 בנחיתה):
...
ומחוברת לענף הקפיצה ב-`compute_piece_views`, עם הגנה כך שכלי שקופץ
מהשורה **העליונה ביותר** בלוח (אין שם מקום מעל הקנבס) לעולם לא יצויר
מחוץ לתמונה — הוא פשוט לא עולה בשורה הזו, מגבלה קטנה אבל אמיתית שסימנתי
לך במקום לתקן בלי לשאול (התיקון האמיתי דורש מרווח בקנבס + שינוי במיפוי
קליקים).

בדקתי ויזואלית (3 תמונות בזמנים שונים) — הכלי עולה, מגיע לשיא, ויורד
בחזרה, כשהצל שכבר מצויר בתוך הספרייט נשאר קבוע על המשבצת.

## תיקון קטן: שימוש עקבי ב-`Img.put_text`

בבדיקה משותפת מצאנו שהודעת סיום המשחק ציירה טקסט עם `cv2.putText` ישירות,
במקום עם `canvas.put_text(...)` (הפונקציה שכבר קיימת ב-`Img` שנתת) — תוקן.
אותה קריאת `cv2` בדיוק מתחת למכסה המנוע, אין הבדל ויזואלי.

## מה עוד שווה לשפר

- **קפיצה מהשורה העליונה לא עולה ויזואלית** - אין מקום מעל הקנבס בלי
  להוסיף מרווח (וזה דורש גם לשנות את מיפוי הקליקים). סימנתי, לא תיקנתי
  בלי לשאול.
...
בכל שלב הרצתי את מלוא סוויטת הטסטים (140, כולל כל אלו שנוספו) וּוידאתי
...
בזמן, הודעת הסיום מוצגת נכון עם שם המנצח, והקפיצה עולה ויורדת בקשת
חלקה.
